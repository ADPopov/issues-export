...
import Table, {
  Table as BaseTable
} from '@jetbrains/ring-ui/components/table/table';
...
import {
  continents,
  countries
} from '@jetbrains/ring-ui/components/table/table.examples2.json';
...
class BasicDemo extends Component {
  ...
  loadPage = () => {
    ...
    let data = [...mock];
    data.sort(
      (a, b) => a[sortKey].localeCompare(b[sortKey]) * (sortOrder ? 1 : -1)
    );
      ...
      <div>
        ...
        <Grid>
          ...
          <Row>
            <Col>
              Active items:{' '}
              {[...selection.getActive()].map(item => item.country).join(', ')}
            </Col>
          </Row>
                  ...
                  <span id="button-select-bulgaria">
                    {selection.isSelected(data[3]) ? (
                      <Button
                        onClick={() =>
                          this.setState({
                            selection: selection.deselect(data[3])
                          })
                        }
                      >
                      ...
                      <Button
                        onClick={() =>
                          this.setState({selection: selection.select(data[3])})
                        }
                      >
                  ...
                  <span id="button-select-finland">
                    ...
                    {selection.isSelected(data[5]) ? (
                      <Button
                        onClick={() =>
                          this.setState({
                            selection: selection.deselect(data[5])
                          })
                        }
                      >
                      ...
                      <Button
                        onClick={() =>
                          this.setState({selection: selection.select(data[5])})
                        }
                      >
